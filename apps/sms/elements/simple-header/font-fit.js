...
(function(exports) {
  ...
  var GaiaHeaderFontFit = {
     ...
     */
    reformatHeading: function(heading, info) {
      ...
      var style = {
        contentWidth: this._getWindowWidth() - info.start - info.end,
        start: info.start,
        end: info.end
      };
    ...
    _centerTextToScreen: function(heading, styleOptions) {
      ...
      var textWidth = styleOptions.textWidth;

      // Get the amount of space on each side of the header text element.
      var tightText = textWidth > (styleOptions.contentWidth - 30);
      var sideSpaceLeft = styleOptions.start;
      var sideSpaceRight = styleOptions.end;
      ...
      if (textWidth + (margin * 2) < this._getWindowWidth() - 1) {
        if (sideSpaceLeft < sideSpaceRight) {
          heading.style.MozMarginStart =
            (sideSpaceRight - sideSpaceLeft) + 'px';
        }
        if (sideSpaceRight < sideSpaceLeft) {
          heading.style.MozMarginEnd = (sideSpaceLeft - sideSpaceRight) + 'px';
        }
      }
    },
  ...
  exports.GaiaHeaderFontFit = GaiaHeaderFontFit;
